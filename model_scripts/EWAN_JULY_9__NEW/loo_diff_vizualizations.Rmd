...
### overall elpd_diff vs. se_diff

```{r overall_diffs, echo=FALSE, message=FALSE}
LOO_DIFF_FILE <-readr::read_csv("loo_diff_with_matches.csv")
library(ggplot2)
ggplot(LOO_DIFF_FILE, aes(x=elpd_diff, y=se_diff)) +
  geom_point()
...
ggplot(LOO_DIFF_FILE, aes(x=as.factor(NUM_MATCHES), y=elpd_diff)) +
  geom_violin() +
...
```{r, echo=FALSE}
ggplot(LOO_DIFF_FILE, aes(x=as.factor(NUM_MATCHES), y=se_diff)) +
  geom_violin() +
  xlab("Number of coefficients for which the model matches the sign of the coeff as sampled")
...
```{r, echo=FALSE}
ggplot(LOO_DIFF_FILE, aes(x=num_zeroes, y=se_diff)) +
  geom_violin() +
  xlab("Number of coefficients sampled based on a zero effect")+
  scale_x_discrete(limits=c("zero","one","two","three"))
```
